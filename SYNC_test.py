...
filename = "boston,_ma_ttfb_rfailures.parquet"
...
for city, late in delay.items():
	...
	it+=1
	ax.set_xlim(0, data_pair_list[0][0][100])
	ax.set_ylim(200, 400)
	# metadata = dict(title="Movie", artist="sourabh")
	writer = anime.PillowWriter(fps=40)
	with writer.saving(fig, os.path.join('./', 'speedtest'+'.gif'), 100):
